...
class EventoDetalhes : AppCompatActivity() {
    ...
    private lateinit var txtEventoStatus: TextView

    private lateinit var databaseReference: DatabaseReference

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_dlf_evento_detalhes)
        enableEdgeToEdge()
        ...
        txtEventoStatus = findViewById(R.id.textView29)
                    ...
                    if (snapshot.exists()) {
                        for (eventSnapshot in snapshot.children) {
                            ...
                            try {
                                ...
                                if (dataEventoDate != null && dataEventoDate.before(dataAtual)) {
                                    txtEventoStatus.text = "Evento Passado"
                                    txtEventoStatus.setBackgroundResource(R.drawable.light_red_bg)
                                    txtEventoStatus.setTextColor(
                                        ContextCompat.getColor(this@EventoDetalhes, R.color.vermelho)
                                    )
                                } else {
                                    txtEventoStatus.text = "Evento Futuro"
                                    txtEventoStatus.setBackgroundResource(R.drawable.light_green_bg)
                                    txtEventoStatus.setTextColor(
                                        ContextCompat.getColor(this@EventoDetalhes, R.color.verde_app)
                                    )
                                }
                            } catch (e: Exception) {
                                Log.e("ParseData", "Erro ao formatar data: ${e.message}")
                            }
